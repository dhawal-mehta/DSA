...
class Solution:
    ...
    def solve(self, A):
        ...
        def inside(x , y):
            return True if x >=0 and x < len(A) and y >= 0 and y < len(A[0]) else False
        
        def util(startx  ,starty):
            de = deque()
            de.append([startx, starty])
            A[startx ][starty] = 0
            ...
            while de:
                ...
                if inside(x-1 ,y) and A[x-1 ][y] == 1:
                    de.append([x -1,y])
                    A[x-1][y] = 0
                    
                if inside(x ,y-1) and A[x ][y-1] == 1:
                    de.append([x ,y-1])
                    A[x][y-1] = 0
                ...
                if inside(x-1 ,y-1) and A[x-1 ][y-1] == 1:
                    de.append([x-1 ,y-1])
                    A[x-1][y-1] = 0
